...
async def list_exercises(
    ...
    async with pool.acquire() as conn:
        ...
        result = []
        for r in rows:
            raw_contraindications = r.get("contraindications") or ""
            if isinstance(raw_contraindications, str):
                contra_list = [x.strip() for x in raw_contraindications.split(",") if x.strip()]
            else:
                contra_list = raw_contraindications
            result.append({
                "id": str(r["id"]),
                "name": r["name"],
                "level": r["level"],
...
async def create_exercise(req: ExerciseCreate):
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            ...
            req.name,
            req.level,
            ",".join(req.contraindications) if req.contraindications else "",
            req.video_url,
        )
...
async def get_exercise(exercise_id: str):
    ...
    pool = await get_pool()
    async with pool.acquire() as conn:
        row = await conn.fetchrow("SELECT * FROM exercises WHERE id = $1", int(exercise_id))
        if not row:
            raise HTTPException(status_code=404, detail="Exercise not found")
        raw_contraindications = row.get("contraindications") or ""
        if isinstance(raw_contraindications, str):
            contra_list = [x.strip() for x in raw_contraindications.split(",") if x.strip()]
        else:
            contra_list = raw_contraindications
...
async def update_exercise(exercise_id: str, req: ExerciseUpdate):
    ...
    pool = await get_pool()
    async with pool.acquire() as conn:
        existing = await conn.fetchrow("SELECT * FROM exercises WHERE id = $1", int(exercise_id))
        if not existing:
            raise HTTPException(status_code=404, detail="Exercise not found")
        updates = {}
        ...
        if req.contraindications is not None:
            updates["contraindications"] = ",".join(req.contraindications) if req.contraindications else ""
        ...
        if updates:
            ...
            values.append(int(exercise_id))
            await conn.execute(
                f"UPDATE exercises SET {', '.join(set_clauses)} WHERE id = ${idx}",
                *values,
            )
            return {"detail": "Exercise updated", "updated": True}
...
async def delete_exercise(exercise_id: str):
    ...
    pool = await get_pool()
    async with pool.acquire() as conn:
        result = await conn.execute("DELETE FROM exercises WHERE id = $1", int(exercise_id))
        if result == "DELETE 0":
            raise HTTPException(status_code=404, detail="Exercise not found")
...
async def upload_video(exercise_id: str, file: UploadFile = File(...)):
    ...
    pool = await get_pool()
    async with pool.acquire() as conn:
        await conn.execute("UPDATE exercises SET video_url = $1 WHERE id = $2", video_url, int(exercise_id))
