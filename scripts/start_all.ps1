...
if (-not (Test-Path $frontendDist)) {
    ...
    Write-Host "Frontend dist found — skipping build"
}
...
Write-Host ""
Write-Host "All services started. Frontend served by gateway at http://localhost:8000"
Write-Host ""
Write-Host "For development with hot-reload, run separately:"
Write-Host "  cd frontend && npm run dev"
Write-Host "  (frontend at http://localhost:5173 → proxies API to gateway)"
